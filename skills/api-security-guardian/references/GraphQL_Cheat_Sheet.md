...
[ OWASP/CheatSheetSeries  ](https://github.com/OWASP/CheatSheetSeries "Go to repository")
  ...
  * Cheatsheets  Cheatsheets 
    ...
    * GraphQL  [ GraphQL  ](GraphQL_Cheat_Sheet.html) Table of contents 
      * Introduction 
      * Common Attacks 
...
This Cheat Sheet provides guidance on the various areas that need to be considered when working with GraphQL:
  ...
  * Apply proper [input validation](Input_Validation_Cheat_Sheet.html) checks on all incoming data.
  * Expensive queries will lead to [Denial of Service (DoS)](Denial_of_Service_Cheat_Sheet.html), so add checks to limit or prevent queries that are too expensive.
  * Ensure that the API has proper [access control](Access_Control_Cheat_Sheet.html) checks.
...
See the OWASP Cheat Sheets on [Input Validation](Input_Validation_Cheat_Sheet.html) and general [injection prevention](Injection_Prevention_Cheat_Sheet.html) for full details to best perform input validation and prevent injection.
...
Validate all incoming data to only allow valid values (i.e. allowlist).
  ...
  * Gracefully [reject invalid input](Error_Handling_Cheat_Sheet.html), being careful not to reveal excessive information about how the API and its validation works.
...
For more information see the below pages:
  ...
  * [LDAP Injection Prevention](LDAP_Injection_Prevention_Cheat_Sheet.html)
  * [OS Command Injection Prevention](OS_Command_Injection_Defense_Cheat_Sheet.html)
  * [XML Security](XML_Security_Cheat_Sheet.html) and [XXE Injection Prevention](XML_External_Entity_Prevention_Cheat_Sheet.html)
...
However, containerization platforms tend to make this task much easier. See the resource limiting section in the [Docker Security Cheat Sheet](Docker_Security_Cheat_Sheet.html#rule-7-limit-resources-memory-cpu-file-descriptors-processes-restarts) for how to prevent DoS when using containers.
